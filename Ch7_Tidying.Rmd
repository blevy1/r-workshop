...
View(inverts)  #shows whole table
names(inverts) #shows top line
summary(inverts) #summarizes table
```
...
(I think wide format has data spliot across multiple columns while long format collapses them into fewer columns)
...
```{r}
class(inverts_long$year) #to determine the type of data (nu)
```
...
inverts_wide <- inverts_wide %>% 
 ...
 # clean_names( "lower_upper") #another case option (see below)
...
inverts_moyr <- inverts_long %>% 
  unite(col = "mo_year", # What to name the new united column
